var UpFieldForm = 
{
	doLoad : function()
	{
		var conditionalFields = [
  			"FConstant", "FExpression", "FDownFieldId",  "FJava", "FParameter", "FSql",
  			"FUnit",
  		...
  		if(form.hasControl("FSourceType")){
  			Jui.event.attach(form.getControl("FSourceType"), "onchange", UpFieldForm.doSourceTypeBoxChange);
  			form.getControl("FSourceType").fireEvent("onchange");
  		}
  		if(form.hasControl("FType")){
  			Jui.event.attach(form.getControl("FType"), "onchange", UpFieldForm.doTypeBoxChange);
  			form.getControl("FType").fireEvent("onchange");
  		}
  		if(form.hasControl("FHandle")){
  			Jui.event.attach(form.getControl("FHandle"), "onchange", UpFieldForm.doSpecialChange);
	...
	doSourceTypeBoxChange : function()
	{
		...
		form.setFieldEnabled("FConstant", sourceType == 'Constant' || sourceType == 'DownField', true);
...
Jui.event.attach(window, 'load', UpFieldForm.doLoad);