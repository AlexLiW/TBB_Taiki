var UDebitLoss_Report = {
	doLoad : function()
	{
	    ...
	    //金融卡類別hsin
        if(tbb.U_LossCardType==0){
            document.getElementsByName("LossCardType")[0].checked="checked";
        }else if(tbb.U_LossCardType==1){
            document.getElementsByName("LossCardType")[1].checked="checked";
        }else if(tbb.U_LossCardType==2){
            document.getElementsByName("LossCardType")[2].checked="checked";
        }else if(tbb.U_LossCardType==3){
            document.getElementsByName("LossCardType")[3].checked="checked";
        }else if(tbb.U_LossCardType==4){
            document.getElementsByName("LossCardType")[4].checked="checked";
        }
        
        //金融卡狀態hsin
        if(tbb.U_LossKind_New==1){
            document.getElementsByName("LossKind")[0].checked="checked";
        }else if(tbb.U_LossKind_New==2){
            document.getElementsByName("LossKind")[1].checked="checked";
        }else if(tbb.U_LossKind_New==3){
            document.getElementsByName("LossKind")[2].checked="checked";
        }
		
       //掛失種類hsin
        if(tbb.U_LossType!=null){
            var s = tbb.U_LossType.split(",");
            for(var i=0;i<s.length;i++){
                document.getElementsByName("DebitCardTerminal")[s[i]].checked="checked";
            }
        }
