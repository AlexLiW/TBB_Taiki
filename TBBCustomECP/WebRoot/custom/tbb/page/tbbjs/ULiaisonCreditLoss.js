var ULiaisonCreditLoss = {
		doLoad : function()
		{
			ULiaisonCreditLoss.doIsNew();
			ULiaisonCreditLoss.getContactInfo();
			ULiaisonCreditLoss.setOnChange();
			ULiaisonCreditLoss.getStatus();
			ULiaisonCreditLoss.setRequire();
			ULiaisonCreditLoss.setLossInfo();//是否补卡
			ULiaisonCreditLoss.setWhetherCardReplace();
			ULiaisonCreditLoss.setRequire1();
			ULiaisonCreditLoss.setRequire2();
			ULiaisonCreditLoss.doFireOnchange();
			...
			if (clientData.entityId != null ) {
				ULiaisonCreditLoss.addServiceTrack();
			}
		...
		{	
			//form.getControl("U_Notifiers").onchange=ULiaisonCreditLoss.setRequire;			
			form.getControl("U_CallBackStaff1").onchange=ULiaisonCreditLoss.setRequire;
			form.getControl("U_IfSend").onchange=ULiaisonCreditLoss.setRequire;
			form.getControl("U_LocationOccur").onchange=ULiaisonCreditLoss.setRequire;
			form.getControl("U_LossReason").onchange=ULiaisonCreditLoss.setRequire;			
			form.getControl("U_Remark").onchange=ULiaisonCreditLoss.setRequire;			
			form.getControl("U_Notifiers").onchange = ULiaisonCreditLoss.setControl; // 通報人
			form.getControl("U_CardSort").onchange = ULiaisonCreditLoss.setControl;//卡别
			form.getControl("U_SupplyCard").onchange = ULiaisonCreditLoss.setLossInfo;//是否补卡
			form.getControl("U_LossInform").onchange=ULiaisonCreditLoss.setRequire1;//挂失告知
						
		},
	//控制是否补卡，和挂失告知
		setControl : function()
		{
			ULiaisonCreditLoss.setWhetherCardReplace();//是否补卡
			ULiaisonCreditLoss.setLossInfo();
		},
   //新建狀態頁面
		doIsNew : function()
		{
...
//是否补卡
		setWhetherCardReplace : function(){
			...
			var Cardholder=form.getFieldValue("U_Cardholder");	//持卡人
			var items= form.getControl("U_SupplyCard").getItems();
			var item= form.getControl("U_LossInform").getItems();
			console.log(items);
			
			if((Notifiers==0 || Notifiers==2)&& CardSort==0)
			    //通报人 0,2 卡别 0 是否补卡 0,2,7 , 20170224 add by chainsea\alex.liwu
                //通報人 0,2 卡別 0 , 是否补卡 7,8 , 20210531 add by chainsea\hsin.lin 新規則調整
			{				
				for(var i=0;i<items.length;i++){
				    if(items[i].value== "7"||items[i].value== "8"){
			...
			{
				for(var i=0;i<items.length;i++){
					if(items[i].value== "2" || items[i].value== "7"|| items[i].value== "8"){
							items[i].disabled=false;			
					}
					else{
						items[i].disabled=true;
	...
	//控制掛失告知
		 setLossInfo : function()
		{
			...
			else if((Notifiers==0 || Notifiers==2)&& (CardSort==0||CardSort==3)&&(SupplyCard==2 || SupplyCard==3 || SupplyCard==4 ||SupplyCard==8))//通报人 0,2 卡别 0,3 是否补卡2,3,4   //2&4
			{				
				for(var i=0;i<items.length;i++){
			...
			else if((Notifiers==0 || Notifiers==2)&& CardSort==4 && (SupplyCard==2 || SupplyCard==3 || SupplyCard==4 ||SupplyCard==8))//通报人 0,2 卡别 4 是否补卡2,3,4  //6
			{				
				for(var i=0;i<items.length;i++){
