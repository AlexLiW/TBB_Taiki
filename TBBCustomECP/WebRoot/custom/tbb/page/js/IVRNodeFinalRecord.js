var IVRNodeFinalRecord = {
	doLoad: function() {
		...
		Utility.invoke("Ecp.CallLog.getListData", CallLog_QueryFilter, true, function(ret) {
			...
			if (ret.data.records.length <= 0) { //判斷回傳筆數<=0
				...
				IVRNodeFinalRecord.doSearchRecord(ret.data.records[0].FCallerPIN);
			} else {
				console.log("Search Fail. Ecp.CallLog.getListData is not exists. ");
	...
	doSearchRecord: function(CallerPIN) {
		var QueryFilter = {
					fieldNames: ["Seq","NodeRecord","CallerPIN","FCreateTime"], //回傳的欄位
					conditions: [{
		...
		Utility.invoke("TBBmIVRNodeFinalRecord.getListData", QueryFilter, true, function(ret) {
			console.log("TBBmIVRNodeFinalRecord.getListData res = " + JSON.stringify(ret ));
			if (ret.data.records.length <= 0) { //判斷回傳筆數<=0
				console.log("Search Fail. TBBmIVRNodeFinalRecord.getListData is not exists. ");	
			} else {
				console.log("TBBmIVRNodeFinalRecord.getListData Search Complet. ");
