...
var UDebitLoss = {
		...
		{			
			...
			//form.setFieldDisabled("U_CustAddress",false); //住家
				...
				UDebitLoss.setRequire();
				//UDebitLoss.setRequire4(); 20210618 hsin 改寫發電文方式
				//UDebitLoss.setRequire4New();  //2021.11.16 Wolf wu 註解掉此動作避免doLoad時均會執行一次S102，改為抓取暫存欄位將帳號資料塞回下拉框中
				UDebitLoss.checkDebitCardLoss();
				var AccountNumberSystemTemp = form.getFieldValue("U_AccountNumberSystemTemp");
				AccountNumberSystemTemp = JSON.parse(AccountNumberSystemTemp);
				form.getControl("U_AccountNumberSystem").loadItems(AccountNumberSystemTemp);
			}
		...
		getContactInfo : function()
        {
            ...
            if(fid==null){
                ...
                if(args.hasOwnProperty("U_CustID")){
                    form.setFieldValue("U_CustID",args.U_CustID); // ID
                }
                
                if(args.hasOwnProperty("U_UserName")){
                   form.setFieldValue("U_CustName",args.U_UserName); // 客戶姓名
                }
                if(args.hasOwnProperty("U_Moblie")){
                   form.setFieldValue("U_CustMobile",args.U_Moblie); // 手机
                }
                if(args.hasOwnProperty("U_Address2")){
                   form.setFieldValue("U_CustAddress",args.U_Address2); // 住家
                } 
                if(args.hasOwnProperty("U_Phone")){
                       form.setFieldValue("U_TelNum",args.U_Phone); // 住家电话
                }
            }
		...
		checkDebitCardLoss:function()
		{
			...
			//var U_LossKind=form.getFieldValue("U_LossKind"); //掛失種類 new 20210618 hsin
			var U_LossKind=form.getFieldValue("U_LossType");
...
//				toolBar.setItemDisabled("DebitCardLoss",false);
			...
			var U_LossType=form.getFieldValue("U_LossType");
			
	         if(U_AccountNumberSystem=='請選擇帳號' ||  U_LossType==null ){
	             toolBar.setItemDisabled("DebitCardLoss",true);
	         }else{
		...
		setOnChange : function()
		{
			...
			//20180927      防呆設置   susie 
			//20210618 hsin 替換欄位
			Jui.event.attach(form.getControl("U_LossType"),'onchange',function(){
				UDebitLoss.checkDebitCardLoss();
				UDebitLoss.setRequire1();
			}); 
			...
			form.getControl("U_LossType").onchange = UDebitLoss.checkDebitCardLoss; //掛失種類 new 20210618 hsin
			//form.getControl("U_AccountNumberSystem").onchange = UDebitLoss.checkDebitCardLoss;//帳號
			form.getControl("U_AccountNumberSystem").onchange = UDebitLoss.AcctoA004; //修改帳號變更時讀取的動作方式 AI.Wolf 20211116
			/*Jui.event.attach(form.getControl("U_AccountNumberSystem"),'onchange',function(){
			  //選擇帳號後發動A004，取得悠遊卡卡號(M.A004.SAMDKEY) 20211003 hsin
                UDebitLossForm.doAccChange();
				UDebitLoss.checkDebitCardLoss();
				setTimeout(function(){
					EntityForm.$doSave();
				}, 1000);
		...
		AcctoA004 : function () { //修改帳號變更時讀取的動作方式 AI.Wolf 20211116
			  //選擇帳號後發動A004，取得悠遊卡卡號(M.A004.SAMDKEY) 20211003 hsin //將此動作獨立成function
                UDebitLossForm.doAccChange();
				UDebitLoss.checkDebitCardLoss();
				setTimeout(function(){
					//發完後先自動保存
		...
		setRequire : function()
		{
			...
			if(U_Notifiers== 0) // 本人
			{
				...
				form.setFieldValue("U_PhoneInform",0+","+1+","+PhoneInform);
		...
		{//掛失類別選擇/金融卡端/任一選項，則/掛失種類/為必填
		    ...
		    //20210618 hsin 改成判斷掛失種類 U_LossType
		...
		getStatus : function(){
			...
			if(status=="AgentSign1"){
				...
				form.setFieldDisabled("U_LossType",true); //掛失種類 new 20210618 hsin
		...
		doSave:function()
		{
			...
			//改成判斷新欄位U_LossType hsin
			if(U_LossType==null)
			{
				Jui.message.alert("請至少選擇一種掛失種類");
				return;
			}
			else
			{
