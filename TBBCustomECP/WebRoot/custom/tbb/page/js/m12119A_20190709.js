...
//			toolBar.getItem("unClose").setDisabled(true);
		...
		setF_Unallocated :function()
		{
			...
			if(F_Unallocated<0){
			    ...
			    isValid= false;
					...
					if(F_TotalMoney!=null && F_TotalMoney<3000){
						Jui.message.alert("分期總金額大於3000才可以保存");
						isValid= false;
					}else{
						isValid= true;
					}
		...
		setF_ApplyInstallVisible : function(){
			...
			if(F_ApplyInstall==1){
				...
				form.setFieldRequired("F_Unallocated",true);
		...
		doOpen : function()
		{
			...
			var F_DepartmentId                	= form.getFieldText("F_DepartmentId");//處理部門
                        var F_Installmentdate		         =  form.getFieldValue("F_Installmentdate");//	
			...
			var args = {
				...
				F_ApplyInstall		: F_ApplyInstall,
                                F_Installmentdate       : F_Installmentdate
			};
