...
//是否补卡
//SupplyCard add:8 remove:5  20210531 add by chainsea\hsin.lin 
		setWhetherCardReplace : function(){
			var Notifiers=form.getFieldValue("U_Notifiers"); //通報人
			var CardSort=form.getFieldValue("U_CardSort"); //卡別
			var SupplyCard=form.getFieldValue("U_SupplyCard");	//是否補卡			
	...
	//控制掛失告知
		 setLossInfo : function()
		{
			...
			else if((Notifiers==0 || Notifiers==2)&& (CardSort==0||CardSort==3)&&(SupplyCard==2 || SupplyCard==3 || SupplyCard==4 ||SupplyCard==8))//通报人 0,1,2 卡别 0,3 是否补卡2,3,4   //2&4
			{				
				for(var i=0;i<items.length;i++){
			...
			else if(Notifiers==1 && CardSort==0 && SupplyCard==2 || SupplyCard==8)//通报人1 卡别 0 是否补卡2 8  //15
			{				
				for(var i=0;i<items.length;i++){
					if(items[i].value== "2" || items[i].value== "3" || items[i].value== "4" || items[i].value== "5" || items[i].value== "8" || items[i].value== "12"){
						items[i].disabled=false;					
			...
			/*
			else if(SupplyCard==8){    //是否補卡8 20210531 add by chainsea\hsin.lin
				for(var i=0;i<items.length;i++){
					if(items[i].value== "0" || items[i].value== "1" || items[i].value== "2" || items[i].value== "3" || items[i].value== "4" || items[i].value== "5" || items[i].value== "8"|| items[i].value== "12"){
						items[i].disabled=false;					
					}else{
		...
		//列印(新)  20210531 add by chainsea\hsin.lin 
		doNewOpen : function()
		{
			var U_CustID                	= form.getFieldValue("U_CustID");//身份證號碼統一編碼	
			var U_CardKind              	= form.getFieldValue("U_CardKind");//卡種
		...
		//	var U_LossInform                = form.getFieldValue("U_LossInform");//挂失告知
			...
			Utility.openDialog("TBB.UCreditLoss.ReportNew.page",args);
		},
